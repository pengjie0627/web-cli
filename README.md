...
```js
// 根据模板初始化项目
zzhd-web-cli create project-name
// 初始化配置文件
zzhd-web-cli config set repo repo-name
```
...
- package.json中设置在命令下执行zzhd-web-cli时调用bin目录下的www文件

  ```js
  "bin": { 
      "zzhd-web-cli": "./bin/www"
   }
  ```
...
​       测试成功的命令行中使用zzhd-web-cli命令,并且可以执行main.js文件
...
const mapActions = {
  ...
  config: {
    ...
    examples: [
      'zzhd-web-cli config set <k><v>',
      'zzhd-web-cli config get <k>',
    ],
  },
  '*': {
    alias: '',
...
执行zzhd-web-cli  create  project 可以打印出project
...
const fetchRepoList = async () => {
  ...
  const {
    data,
  } = await axios.get('https://api.github.com/orgs/zzhd-web-cli/repos');
  return data;
};
...
const fetchTagList = async (repo) => {
  ...
  const {
    data,
  } = await axios.get(`https://api.github.com/repos/zzhd-web-cli/${repo}/tags`);
  return data;
};
// 获取对应的版本号
...
downloadGitRepo这个方法不是promise方法,没关系我们自己包装下
...
let api = `zzhd-web-cli/${repo}`;  // 下载项目 
if (tag) {
	api += `#${tag}`;
}
